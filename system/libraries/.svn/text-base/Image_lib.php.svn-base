...
class CI_Image_lib {
	...
	function initialize($props = array())
	{
		...
		/*
		 *Is the sae storage domain empty?
		 *
		 *first set the sae storage domain
		 *and value the stor_domain
		 *
		 */
		if(defined('SAE_TMP_PATH')){
			if($this->stor_domain == ''){
				$this->set_error('imglib_storage_empty');
				return false;
			}
		}
	...
	function image_save_gd($resource)
	{
		switch ($this->image_type)
		{
			case 1 :
						...
						if(defined('SAE_TMP_PATH')){
							if(!imagegif($resource, SAE_TMP_PATH.'test.gif')){
								$this->set_error('imglib_save_failed');
								return false;
							}

							if(!($storage = new SaeStorage())){
								$this->set_error('imglib_storage_init_failed');
								return false;
							}

							if(!$storage->upload($this->stor_domain, $this->full_dst_path, SAE_TMP_PATH."test.gif")){
								$this->set_error('imglib_storage_upload_failed');
								return false;
							}
						}
			...
			case 2	:
						...
						if(defined('SAE_TMP_PATH')){
							if(!imagejpeg($resource, SAE_TMP_PATH.'test.jpg', $this->quality)){
								$this->set_error('imglib_save_failed');
								return false;
							}
							...
							if(!$storage->upload($this->stor_domain, $this->full_dst_path, SAE_TMP_PATH."test.jpg")){
								$this->set_error('imglib_storage_upload_failed');
								return false;
							}
						}else{
			...
			case 3	:
						...
						if(defined('SAE_TMP_PATH')){
							if(!imagepng($resource,SAE_TMP_PATH."test.png")){
								$this->set_error('imglib_save_failed');
								return false;
							}
							...
							if(!$storage->upload($this->stor_domain,$this->full_dst_path, SAE_TMP_PATH."test.png")){
								$this->set_error('imglib_storage_upload_failed');
								return false;
							}
						}else{	
